...
- استخدم جداول ونقاط مختص������ة بدل الفقرات الطويلة
...
const aiTools = [
  ...
  mkTool("import_tools_from_github", "استيراد أدوات مخصصة من ملف JSON على GitHub — يجلب الملف ويضيف كل الأدوات تلقائياً. يدعم روابط GitHub العادية وraw", 
    ...
    ["github_url"]),
  mkTool("execute_github_tool", "تنفيذ أداة مستوردة من GitHub بشكل فعلي مع دقة عالية — تدعم JavaScript, Python, Bash, Go", 
    { tool_id: { type: "string", description: "معرف الأداة المستوردة من GitHub" }, 
      params: { type: "string", description: "معاملات التنفيذ بصيغة JSON مثل: '{\"target\":\"example.com\"}'" },
      use_cache: { type: "string", description: "استخدام الذاكرة المؤقتة (true/false) — اختياري" } },
    ["tool_id"]),
  // FILE SENDING
  ...
  mkTool("save_scan_result", "حفظ نتيجة فحص في ��لذ��كرة للرجوع إليها لاحقاً", 
...
async function importToolsFromGitHub(args: Record<string, string>): Promise<string> {
  try {
    ...
    for (const t of tools) {
      ...
      let toolArgs = t.args || t.args_def || t.argsDef || [{ key: "target", label: "ال��دف", placeholder: "example.com", required: true }];
...
async function executeToolCall(name: string, args: Record<string, string>): Promise<string> {
  ...
  if (name === "execute_github_tool") {
    try {
      const { tool_id, params: paramsStr, use_cache } = args;
      if (!tool_id) return "❌ يجب تقديم tool_id";
      
      let params: Record<string, unknown> = {};
      try {
        params = paramsStr ? (typeof paramsStr === "string" ? JSON.parse(paramsStr) : paramsStr) : {};
      } catch {
        return "❌ صيغة معاملات JSON غير صحيحة";
      }

      const useCache = use_cache !== "false"; // Default to true
      
      // Call the execution API
      const execResp = await fetch(`${SUPABASE_URL}/functions/v1/execute-github-tool`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": `Bearer ${SUPABASE_ANON_KEY}` },
        body: JSON.stringify({ tool_id, params, useCache }),
      });

      if (!execResp.ok) {
        const error = await execResp.json();
        return `❌ فشل التنفيذ: ${error.error || execResp.statusText}`;
      }

      const result = await execResp.json();
      
      if (!result.success) {
        return `❌ خطأ في التنفيذ:\n${result.error}\n⏱️ الوقت: ${result.executionTimeMs}ms`;
      }

      const output = typeof result.output === "string" ? result.output : JSON.stringify(result.output, null, 2);
      const cacheInfo = result.cached ? " (من الذاكرة المؤقتة)" : "";
      const statsInfo = result.stats ? `\n📊 إحصائيات:\n   ✅ نجح: ${result.stats.successCount}/${result.stats.totalExecutions}\n   ⏱️ متوسط الوقت: ${result.stats.avgExecutionTime}ms` : "";
      
      return `✅ تم التنفيذ بنجاح${cacheInfo}\n\n📋 النتيجة:\n${output}\n⏱️ وقت التنفيذ: ${result.executionTimeMs}ms${statsInfo}`;
    } catch (e) {
      return `❌ خطأ: ${e instanceof Error ? e.message : "خطأ"}`;
    }
  }
...
const PROVIDER_CONFIGS: Record<string, { baseUrl: string; authHeader: (key: string) => Record<string, string>; isAnthropic?: boolean }> = {
  ...
  gemini: { baseUrl: "https://generativelanguage.googleapis.com/v1beta/openai/chat/completions", authHeader: (k) => ({ Authorization: `Bearer ${k}` }) },
...
async function callAI(messages: any[], tools: any[], stream: boolean, customProvider?: { providerId: string; modelId: string; apiKey: string; apiKeys?: string[] }) {
  // Special handling for Gemini Python - no API key needed, direct connection
  if (customProvider?.providerId === "gemini" && customProvider.modelId === "gemini-python") {
    try {
      ...
      await process.stdin?.write(new TextEncoder().encode(messagesJson));
      ...
      if (!status.success) {
        throw new Error(`Python script failed: ${new TextDecoder().decode(await process.stderrOutput())}`);
      }
      ...
      // Return as Response object for compatibility
      return new Response(JSON.stringify(responseData), {
        status: 200,
        headers: { "Content-Type": "application/json" }
      });
    } catch (e) {
      console.error("[v0] Gemini Python error:", e);
      throw new Error(`خطأ في تنفيذ Python Gemini: ${e instanceof Error ? e.message : String(e)}`);
    }
  }
...
serve(async (req) => {
  ...
  try {
    ...
    // Exception: Gemini Python doesn't need API key
